...
>(
  ...
  return useQuery<InputType<GraphQLTypes[O], Z, SCLR>>(
    gql(
      Zeus("query", query, {
...
>(
  ...
  return useLazyQuery<InputType<GraphQLTypes[O], Z, SCLR>>(
    gql(
      Zeus("query", LazyQuery, {
        operationOptions: options?.operationOptions,
        scalars: options?.scalars,
      })
    ),
...
>(
  ...
  return useMutation<InputType<GraphQLTypes[O], Z, SCLR>>(
    gql(
      Zeus("mutation", mutation, {
        operationOptions: options?.operationOptions,
        scalars: options?.scalars,
      })
    ),
